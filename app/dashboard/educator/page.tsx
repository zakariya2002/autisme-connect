        ...
        {kpis && (
          <div className="mt-4 sm:mt-6 lg:mt-8 px-3 sm:px-4 lg:px-0">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3">
              {/* Revenus du mois */}
              <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-16 h-16 -mr-6 -mt-6 rounded-full opacity-10" style={{ backgroundColor: '#41005c' }} />
                <div className="flex items-center gap-2 mb-1.5 relative">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: '#f3e8ff' }}>
                    <svg className="w-3.5 h-3.5" style={{ color: '#41005c' }} fill="none" stroke="currentColor" strokeWidth={2.2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                  </div>
                  <p className="text-[11px] sm:text-xs text-gray-500 font-medium">Revenus du mois</p>
                </div>
                <p className="text-lg sm:text-2xl font-extrabold relative" style={{ color: '#41005c' }}>{formatEuro(kpis.revenueMonth)}</p>
                <p className="text-[10px] sm:text-[11px] text-gray-400 mt-0.5">{kpis.sessionsMonth} séance{kpis.sessionsMonth > 1 ? 's' : ''}</p>
              </div>

              {/* Vues 30j */}
              <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-16 h-16 -mr-6 -mt-6 rounded-full opacity-10 bg-pink-500" />
                <div className="flex items-center gap-2 mb-1.5 relative">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: '#fce7f3' }}>
                  ...
                  <p className="text-[11px] sm:text-xs text-gray-500 font-medium">Vues 30 j</p>
                </div>
                <p className="text-lg sm:text-2xl font-extrabold text-pink-600 relative">{kpis.profileViews30}</p>
                <p className="text-[10px] sm:text-[11px] text-gray-400 mt-0.5">de votre profil</p>
              </div>

              {/* RDV à venir */}
              <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-16 h-16 -mr-6 -mt-6 rounded-full opacity-10 bg-emerald-500" />
                <div className="flex items-center gap-2 mb-1.5 relative">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ backgroundColor: '#d1fae5' }}>
                    <svg className="w-3.5 h-3.5 text-emerald-600" fill="none" stroke="currentColor" strokeWidth={2.2} viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                  </div>
                  <p className="text-[11px] sm:text-xs text-gray-500 font-medium">RDV à venir</p>
                </div>
                <p className="text-lg sm:text-2xl font-extrabold text-emerald-600 relative">{upcomingAppointments.length}</p>
                <p className="text-[10px] sm:text-[11px] text-gray-400 mt-0.5">prochain{upcomingAppointments.length > 1 ? 's' : ''}</p>
              </div>

              {/* Note moyenne */}
              <div className="bg-white rounded-xl border border-gray-100 p-3 sm:p-4 relative overflow-hidden">
                <div className="absolute top-0 right-0 w-16 h-16 -mr-6 -mt-6 rounded-full opacity-10 bg-amber-500" />
                <div className="flex items-center gap-2 mb-1.5 relative">
                  <div className="w-7 h-7 rounded-lg flex items-center justify-center bg-amber-100">
                    <svg className="w-3.5 h-3.5 text-amber-600" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/></svg>
                  </div>
                  <p className="text-[11px] sm:text-xs text-gray-500 font-medium">Note moyenne</p>
                </div>
                <p className="text-lg sm:text-2xl font-extrabold text-amber-600 relative">
