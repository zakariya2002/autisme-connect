    ...
    <header className="fixed top-0 left-0 right-0 z-50" style={{ backgroundColor: '#41005c' }}>
      <div className="max-w-7xl mx-auto px-4 lg:px-8">
        ...
        <div className="hidden lg:flex items-center h-14 xl:h-16">
          ...
          <nav className="flex-1 flex items-center justify-end gap-0.5 xl:gap-1" role="navigation" aria-label="Navigation principale Pro gauche">
            <Link href="/pro/pricing" className={`group flex items-center gap-1 xl:gap-1.5 px-2 xl:px-3 py-1.5 xl:py-2 text-xs xl:text-sm text-white/90 hover:text-white hover:bg-white/15 rounded-md font-medium transition-all whitespace-nowrap ${isActive('/pro/pricing') ? 'bg-white/20 text-white' : ''}`}>
            ...
            <Link href="/pro/how-it-works" className={`group flex items-center gap-1 xl:gap-1.5 px-2 xl:px-3 py-1.5 xl:py-2 text-xs xl:text-sm text-white/90 hover:text-white hover:bg-white/15 rounded-md font-medium transition-all whitespace-nowrap ${isActive('/pro/how-it-works') ? 'bg-white/20 text-white' : ''}`}>
              <svg className="w-3.5 h-3.5 xl:w-4 xl:h-4 opacity-70 group-hover:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white shadow-lg border-t border-gray-100 z-[100]">
          <nav className="px-3 py-3 space-y-0.5" role="navigation" aria-label="Menu principal Pro">
            <Link
              href="/pro/pricing"
            ...
            <Link
              href="/"
              className="block py-2 px-3 rounded-lg text-xs text-center font-semibold transition-all hover:opacity-90 my-2"
              style={{ backgroundColor: '#e6fffa', color: '#027e7e' }}
              onClick={() => setMobileMenuOpen(false)}
            >
              Vous êtes un aidant ?
            ...
            {user ? (
              <Link
                href={getDashboardLink()}
                className="flex items-center justify-center gap-2 py-2 px-3 text-sm text-white rounded-lg text-center font-semibold transition-all hover:opacity-90"
                style={{ backgroundColor: '#f0879f' }}
                onClick={() => setMobileMenuOpen(false)}
              >
              ...
              <>
                <Link
                  href="/pro/login"
                  className="block py-2 px-3 text-sm text-white rounded-lg text-center font-semibold transition-all hover:opacity-90"
                  style={{ backgroundColor: '#f0879f' }}
                  onClick={() => setMobileMenuOpen(false)}
                >
                  Connexion
                ...
                <Link
                  href="/auth/register-educator"
                  className="block py-2 px-3 text-sm text-white rounded-lg text-center font-semibold mt-1.5 transition-all hover:opacity-90"
